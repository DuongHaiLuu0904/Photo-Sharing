...
function LoginRegister() {
    ...
    const handleLogin = async (event) => {
        ...
        try {
            const userData = await authLogin(loginName, loginPassword);
    ...
    const handleRegister = async (event) => {
        ...
        if (!regData.login_name || !regData.password || !regData.first_name || !regData.last_name) {
            setRegError('Login name, password, first name, and last name are required.');
            setRegLoading(false);
            return;
        }
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} sm={6}>
                                <TextField
                                    ...
                                    required
                                    disabled={regLoading}
                                    error={regData.password !== regData.confirmPassword && regData.confirmPassword !== ''}
                                    helperText={regData.password !== regData.confirmPassword && regData.confirmPassword !== '' ? 'Passwords do not match' : ''}
                                />
                            </Grid>
